...
const CONFIG = {
  ...
  GNEWS_KEY: "YOUR_GNEWS_KEY", // <-- replace with your GNews key
...
async function loadMarkets() {
  try {
    const url = `https://www.alphavantage.co/query?function=CURRENCY_EXCHANGE_RATE&from_currency=USD&to_currency=EUR&apikey=${CONFIG.ALPHA_VANTAGE_KEY}`;
    const res = await fetch(url);
    const data = await res.json();
    ...
    if (!data["Realtime Currency Exchange Rate"]) {
      container.innerHTML = "<p>⚠️ Market data unavailable.</p>";
      return;
    }

    const rate = data["Realtime Currency Exchange Rate"]["5. Exchange Rate"];
    container.innerHTML = `<p><strong>USD → EUR:</strong> ${parseFloat(rate).toFixed(2)}</p>`;
  } catch (err) {
    console.error("Market API error:", err);
    document.getElementById("marketData").innerHTML = "<p>⚠️ Could not load markets.</p>";
  }
}
...
async function askAI() {
  ...
  try {
    ...
    const text = data[0]?.generated_text || "🤖 Sorry, I couldn’t generate a response.";
    output.innerHTML = text;
  } catch (err) {
    console.error("AI error:", err);
    output.innerHTML = "⚠️ AI service unreachable. Please check your API key or try again later.";
  }
}
